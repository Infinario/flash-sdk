package com.infinario.api {
	...
	public class Infinario {
		...
		private const DEFAULT_TARGET:String = 'http://api.infinario.com';
		private const TRACKING_COOKIE_KEY:String = '__inf_etc__';
		...
		private function sendPing():void {
			if (config.ping.enabled) {
				this.track('session_ping', this.getPingParams());
			}
		}
		...
		public function initialize(options:Object):void {
			if (initialized) {
				trace('Infinario already initialized');
				return;
			}
            preProcessCustomerIds(options);
			extend(config, options);
			if (!(config.token is String)) {
                trace('Invalid format of Infinario token.');
                return;
            }
            initialized = true;
